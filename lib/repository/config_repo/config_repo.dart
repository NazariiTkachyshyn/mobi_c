...
class ConfigRepo {
  ...
  writeConfig(User user) async {
    ...
    final kasaConfig = user.kasa.toJson();
    final visibleStorageBalance =
        user.visibleStorageBalance.map((e) => e.toJson()).toList();
    configData.addAll({
      'userStorage': user.storage.refKey,
      'odata_responsible_ref': user.user1C.ref,
      'kasa': kasaConfig,
      'visibleStorageBalance': visibleStorageBalance
    });
...
class Config {
  ...
  static List<Storage> get storages {
    final config = _loadConfig();
  ...
  static Kasa get kasa {
    final config = _loadConfig();
    return config.kasa;
  }
    static List<Storage> get visibleStorageBalance {
    final config = _loadConfig();
    return config.visibleStorageBalance;
  }
}
